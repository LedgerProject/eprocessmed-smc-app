...
@Component({
  selector: 'app-lgn-smart-passport',
  templateUrl: './lgn-smart-passport.component.html',
  styleUrls: ['./lgn-smart-passport.component.scss']
})
export class AppLgnSmartPassportComponent implements OnInit {
  ...
  createSession(): void {
    ...
    localStorage.setItem('codePhone', `${this.usrAuth.codePhone}`);
    localStorage.setItem('dni', `${this.usrAuth.dni}`);
    localStorage.setItem('idCatAccesstype', `${this.usrAuth.idCatAccesstype}`);
        ...
        if (msg === 'Authorized' &&  data.idCustomer ==  this.idCustomer) {
          ...
          switch (idCatRoluser) {
            case '0,0,0':// administrator
                routeInit = 'dynamic-forms/list-forms';
              break;
            case '0,1,0':// patient
                if (data.regStatus == 'onhold') {
                  this.openPrivacyPoliciesModal();
                } else {
                  routeInit = 'dynamic-forms/lst-resp-forms';
                }
              break;
            case '0,2,0':// specialist
                routeInit = 'dynamic-forms/medical-dashboard';
              break;
            case '0,3,0':// customer
                routeInit = 'dynamic-forms/lst-resp-forms';
              break;
