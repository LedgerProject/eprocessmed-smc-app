...
const createBuilder = (data) => {
  ...
  reqData.forEach(dta => {
    ...
    const idUserCreate = params.find(param => param.param === 'idUserCreate');
    if (idUserCreate === undefined) {
      params.push({param: 'idUserCreate', value: 0});
    }

    const idUserModify = params.find(param => param.param === 'idUserModify');
    if (idUserModify === undefined) {
      params.push({param: 'idUserModify', value: 0});
    }

    const status = params.find(param => param.param === 'status');
    if (status === undefined) {
      params.push({param: 'status', value: '1'});
    }
